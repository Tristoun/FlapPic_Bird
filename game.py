...
class Game:
    def __init__(self, root):
        ...
        self.reader = None
        if(MODE == "PIC"):
            self.reader = USBDecoder(port="COM8")
            self.reader.start()
        else:
    ...
    def pipes_move(self):
        if(self.state == GameState.RUN):
            ...
            for j in lst_delete:
                ...
                # ENVOI DU SIGNAL AU PIC POUR LE BUZZER
                if(MODE == "PIC" and self.reader):
                    self.reader.send_buzzer_signal()
                    self.reader.send_score(self.score)
    ...
    def click_menu(self, event):
        if(self.state == GameState.MENU):
            if(event.x >= self.bouton_start.x and event.x <= self.bouton_start.x + self.bouton_start.size[0] and 
               event.y >= self.bouton_start.y and event.y <= self.bouton_start.y + self.bouton_start.size[1]):
                self.state = GameState.RUN
                self.canvas.delete(self.bouton_start.id)
                self.player = Bird(self.canvas, path="images/images/dog_wingsdown.png", size=(60,55))
                self.score = 0
